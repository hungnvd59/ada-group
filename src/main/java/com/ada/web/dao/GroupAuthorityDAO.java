...
import com.ada.model.GroupAuthority;
...
public class GroupAuthorityDAO {
    ...
    public Optional<PagingResult> page(String name, PagingResult page) {
        ...
        Long count = (Long) entityManager.createQuery("select count(gr.id) from Group gr where UPPER(gr.groupName) like :name").setParameter("name", "%" + ((name != null && name.length() > 0) ? name.trim().toUpperCase() : "") + "%").getSingleResult();
        List<Group> list = entityManager.createQuery("select gr from Group gr where UPPER(gr.groupName) like :name", Group.class).setParameter("name", "%" + ((name != null && name.length() > 0) ? name.trim().toUpperCase() : "") + "%")
    ...
    public Optional<List<Group>> loadAllGroup() {
        List<Group> items = entityManager.createQuery("Select gr from Group gr ", Group.class).getResultList();
        return Optional.ofNullable(items);
    }

    public Optional<List<Group>> loadAllGroupOfUser(Long userId) {
        List<Group> items = entityManager.createQuery("SELECT gr FROM Group gr JOIN GroupUser gu ON gr.id=gu.groupId where gu.userId=:userId and gr.status=1", Group.class)
    ...
    public Optional<List<User>> loadAllUserOfGroup(Long groupId) {
        List<User> items = entityManager.createQuery("SELECT us FROM User us JOIN GroupUser gu ON us.id=gu.userId where gu.groupId=:groupId and us.status=1", User.class)
     ...
     public Optional<PagingResult> pageGroupByUser(long userId, PagingResult page) {
        ...
        Long count = (Long) entityManager.createQuery("SELECT count(gr) FROM Group gr JOIN GroupUser gu ON gr.id=gu.groupId where gu.userId=:userId and gr.status=1").setParameter("userId", userId).getSingleResult();
        List<Group> list = entityManager.createQuery("SELECT gr FROM Group gr JOIN GroupUser gu ON gr.id=gu.groupId where gu.userId=:userId and gr.status=1").setParameter("userId", userId)
                .setFirstResult(offset).setMaxResults(page.getNumberPerPage()).getResultList();
        if (list != null && count > 0) {
            page.setItems(list);
            page.setRowCount(count.longValue());
     ...
     public Optional<PagingResult> pageUserOfGroup(Long groupId, PagingResult page) {
        ...
        Long count = (Long) entityManager.createQuery("SELECT count(us) FROM User us JOIN GroupUser gu ON us.id=gu.userId where gu.groupId=:groupId and us.status=1").setParameter("groupId", groupId).getSingleResult();
        List<User> list = entityManager.createQuery("SELECT us FROM User us JOIN GroupUser gu ON us.id=gu.userId where gu.groupId=:groupId and us.status=1").setParameter("groupId", groupId)
                .setFirstResult(offset).setMaxResults(page.getNumberPerPage()).getResultList();
        if (list != null && count > 0) {
            page.setItems(list);
            page.setRowCount(count.longValue());
    ...
    public Optional<PagingResult> pageAuthorityOfGroup(long groupId, PagingResult page) {
        ...
        Long count = (Long) entityManager.createQuery("SELECT count(au) FROM Authority au JOIN GroupAuthority ga ON au.id=ga.authority where ga.groupId=:groupId ").setParameter("groupId", groupId).getSingleResult();
        List<Authority> list = entityManager.createQuery("SELECT au FROM Authority au JOIN GroupAuthority ga ON au.id=ga.authority where ga.groupId=:groupId").setParameter("groupId", groupId)
                .setFirstResult(offset).setMaxResults(page.getNumberPerPage()).getResultList();
        if (list != null && count > 0) {
            page.setItems(list);
            page.setRowCount(count.longValue());
...
//            items.stream().forEach(item -> entityManager.persist(item));
    ...
    public Optional<List<String>> loadListAuthorityOfUserByUsername(String username) {
        List<String> items = entityManager.createQuery("SELECT au.authKey FROM Authority au JOIN GroupAuthority ga ON au.id=ga.authority JOIN Group gr ON gr.id=ga.groupId JOIN GroupUser gu ON gr.id=gu.groupId JOIN User us ON us.id=gu.userId WHERE us.username=:username")
    ...
    public List<Group> getAllGroup() {
        List<Group> list = entityManager.createQuery("select a from Group a", Group.class).getResultList();
        return list;
    }
